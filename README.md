...
This repository is a template of an external project-specific scripts directory (PSCD) used to keep custom VQA parameter and import scripts confidential by keeping them outside the main (public) VQA source code directory. Although this template repository is public,  repositories of project-specific scripts would normally be private and accessible only to project administrators and developers. In practice, you would maintain several PCSDs, one for each project. However, you may also keep scripts for different projects in the same PSCD, as long as they belong to the same owner(s). 

The main purpose of the PCSD is to keep project-specific scripts outside of the main source code repository to avoid exposing confidential information publicly. There is no obligation to version-control the PCSD nor to to establish a remote repository, private or otherwise. However, versioning is strongly recommended given the that project-specific scripts are critical components of VQA applications and a remote repository enables automatic distribution of code updates. A private repository for the PCSD components only allows you to keep the main VQA code open source (as required by its GPL 3 license) while restricting access to a bare minimum of truly confidential code.
